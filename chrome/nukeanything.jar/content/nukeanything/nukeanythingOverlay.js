...
var NukeAnything = {
	...
	blink : function blink(node)	{
		...
		function setOutline(o) {
			return function() {node.style.outline = o;}
		}
	...
	doHighlight: function() {
		if(gContextMenu) {
			...
			if(obj) {
				var id = setTimeout( function () {NukeAnything.blink(obj)} , 1000);
			...
			if(obj) {
				if (obj.tagName =='BODY' || obj.tagName =='HTML') {
					...
					if (nodes && nodes[0]) {
						...
						NukeAnything.RemovedObjects[c]={'obj': nodes[0] , 'background-image' : nodes[0].style.backgroundImage} ;
					...
					// go away!
					var c = NukeAnything.RemovedObjects.length;
					NukeAnything.RemovedObjects[c]={'obj': obj , 'display' : obj.style.display} ;
	...
	checkFrames: function(frames, framedoc) {
		...
		for(var f=0; f<frames.length; f++) {
			...
			// go away!
				var c = NukeAnything.RemovedObjects.length;
				NukeAnything.RemovedObjects[c]={'obj': frames[f] , 'display' : frames[f].style.display} ;
